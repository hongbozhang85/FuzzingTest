## Time dependent Epsilon-Greedy Implementation ##
# epsilon is vary from 0.2 to 1(FTL) linearly
# Author: Hongbo Zhang


funcInitVarEPSI = function() {
    ...
    state = list(name="Vary-Epsilon-Greedy",
        filename="vary-epsilon.RData",
        SeedLambdas = Lambdas,
        ...
        epsilon = 0.2,  # 50% exploration, 50% exploitation
        noise = 0.1 )
    return(state)
}

# pull the bandit arm
funcPullVarEPSI = function(state,trial) {
...
funcEstimateVarEPSI = function(state, trial) {
    noise = state$noise
...
## worker assignment ##
funcAssignVarEPSI = function(state, trial) {
    if ((trial > 1) && (trial < duration)) {
		...
		total = sum(state$NumTrials)
        epsilon = state$epsilon + (1-state$epsilon)*total/duration/numworkers
...
### LIST ###
VarEPSILON = list(state=funcInitVarEPSI(), init=funcInitVarEPSI, pull=funcPullVarEPSI, estimate=funcEstimateVarEPSI, assign=funcAssignVarEPSI)

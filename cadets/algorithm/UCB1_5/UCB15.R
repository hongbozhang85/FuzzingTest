...
# Averagereward = 100000 Success/Trials


funcInitUCB15 = function() {
    ...
    state = list(name="UCB1_factor-100000",
        filename="UCB15.RData",
        SeedLambdas = Lambdas,
...
# pull the bandit arm
funcPullUCB15 = function(state,trial) {
...
funcEstimateUCB15 = function(state, trial) {
    noise = state$noise
	...
	total = sum(Trials)
	Estimate = 100000*AverageReward + sqrt(2*log(total)/(Trials+noise))
    state$EstimatedRates = Estimate
    return(state)
}

## worker assignment ##
funcAssignUCB15 = function(state, trial) {
...
### LIST ###
UCB15 = list(state=funcInitUCB15(), init=funcInitUCB15, pull=funcPullUCB15, estimate=funcEstimateUCB15, assign=funcAssignUCB15)

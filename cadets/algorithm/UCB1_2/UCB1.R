...
# Averagereward = Success instead of Success/Trials


funcInitUCB1 = function() {
...
# pull the bandit arm
funcPullUCB1 = function(state,trial) {
...
funcEstimateUCB1 = function(state, trial) {
    noise = state$noise
    Success = state$NumSuccess
    Trials = state$NumTrials
    #Estimate = (Success + noise) / (Trials + noise)
    AverageReward = (Success + noise) # / (Trials + nois)
	total = sum(Trials)
	Estimate = AverageReward + sqrt(2*log(total)/(Trials+noise))
    state$EstimatedRates = Estimate
    return(state)
}

## worker assignment ##
funcAssignUCB1 = function(state, trial) {
...
### LIST ###
UCB1 = list(state=funcInitUCB1(), init=funcInitUCB1, pull=funcPullUCB1, estimate=funcEstimateUCB1, assign=funcAssignUCB1)

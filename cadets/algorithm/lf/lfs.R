## "Least Failure Strategy" Implementation ##
# Choose the process with the least number of failures
# TODO: in case of tie, choose the process with largest number of successes
# Multi-Armed Bandits, Gittins Index, and Its Calculation. P433
#  Hongbo Zhang


funcInitLFS = function() {
    ...
    state = list(name="Least-Failure-Strategy",
        filename="LFS.RData",
        SeedLambdas = Lambdas,
...
# pull the bandit arm
funcPullLFS = function(state,trial) {
...
funcAssignLFS = function(state, trial) {
    if ((trial > 1) && (trial < duration)) {
        ...
        # find the best seed
        bestseed = which.min(state$EstimatedRates)
...
### LIST ###
LFS = list(state=funcInitLFS(), init=funcInitLFS, pull=funcPullLFS, estimate=funcEstimateLFS, assign=funcAssignLFS)

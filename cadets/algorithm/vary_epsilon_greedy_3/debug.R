...
algtrials = 1
makeplots = FALSE
# number of nodes in Markov Chain crash model, index starting from 0 
# e.g. mclength = 1: lambda, gamma*lambda, 0
# in this case, a seed will trigger two unique crashes at most.
# so a seed will trigger (mclength + 1) unique crashes at most.
mclength = 1 
...
ALGORITHMS = list(BAYES, EPSILON, NAIVE, OPTIMAL, FTL)
...
#for (ALG in ALGORI) {
    ...
    for (rep in 1:algtrials) {
        ...
        filename = paste(ALG$state$name, rep, "debug.RData", sep="")
        STORE[[rep]] <- ALG$state

...
# Averagereward = 10000 Success/Trials


funcInitUCB14 = function() {
    ...
    state = list(name="UCB1_factor-10000",
        filename="UCB14.RData",
        SeedLambdas = Lambdas,
...
# pull the bandit arm
funcPullUCB14 = function(state,trial) {
...
funcEstimateUCB14 = function(state, trial) {
    noise = state$noise
	...
	total = sum(Trials)
	Estimate = 10000* AverageReward + sqrt(2*log(total)/(Trials+noise))
    state$EstimatedRates = Estimate
    return(state)
}

## worker assignment ##
funcAssignUCB14 = function(state, trial) {
...
### LIST ###
UCB14 = list(state=funcInitUCB14(), init=funcInitUCB14, pull=funcPullUCB14, estimate=funcEstimateUCB14, assign=funcAssignUCB14)

...
# Averagereward = 1000000 Success/Trials


funcInitUCB16 = function() {
    ...
    state = list(name="UCB1_factor-1000000",
        filename="UCB16.RData",
        SeedLambdas = Lambdas,
...
# pull the bandit arm
funcPullUCB16 = function(state,trial) {
...
funcEstimateUCB16 = function(state, trial) {
    noise = state$noise
	...
	total = sum(Trials)
	Estimate = 1000000*AverageReward + sqrt(2*log(total)/(Trials+noise))
    state$EstimatedRates = Estimate
    return(state)
}

## worker assignment ##
funcAssignUCB16 = function(state, trial) {
...
### LIST ###
UCB16 = list(state=funcInitUCB16(), init=funcInitUCB16, pull=funcPullUCB16, estimate=funcEstimateUCB16, assign=funcAssignUCB16)
